    ...
    .vue-waterfall-easy(:style="isMobile? '' :{width: colWidth*cols+'px',left:'50%', marginLeft: -1*colWidth*cols/2 +'px'}")
      .img-box(
        ...
        component.img-inner-box(
          is="a",
          :data-index="i",
          :style="{ border: outerStyles[i].border, color: outerStyles[i].color }"
          ...
          component.img-wraper(
            ...
            :style="{width:'100%',height:v._height ? v._height+'px': '200px',backgroundColor: !v[srcKey] && outerStyles[i].backgroundColor}"
          )
            img(v-if="v[srcKey]",:src="v[srcKey]")
            h1(v-else class="short-name") {{ shortNames[i] }}
...
<!-- ——————————————↓JS—————————分界线———————————————————————— -->
<script>
import alink from 'vue-waterfall-easy/src/vue-waterfall-easy/components/alink.vue'
...
export default {
  ...
  methods: {
    ...
    preload (src, imgIndex) {
      this.imgsArr.forEach((imgItem, imgIndex) => {
        ...
        if (!imgItem[this.srcKey]) {
          this.imgsArr[imgIndex]._height = '200'
          this.loadedCount++
    ...
    waterfall () {
      ...
      var top; var left; var height; var colWidth = this.isMobile ? this.imgBoxEls[0].offsetWidth : this.colWidth
